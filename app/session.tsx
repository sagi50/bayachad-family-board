...
import StatusEnhancements from './status-enhancements';
import TaskCompletionConfirm from './task-completion-confirm';
...
export default function SessionGate() {
  ...
  if(loggedIn) return <><Board/><QuickShopping/><ShoppingBoardPanel/><EventManager/><StatusEnhancements/><TaskCompletionConfirm/><div className="account-controls"><a href="/api/docs" target="_blank" rel="noreferrer">תיעוד API</a><button className="text-button" onClick={logout} disabled={busy}>התנתקות</button>{error && <span role="alert">{error}</span>}</div></>;
